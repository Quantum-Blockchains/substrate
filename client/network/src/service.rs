	...
	H: ExHashT,
{
	...
	pub fn new(params: Params<B>) -> Result<Self, Error> {
		...
		network_config.pre_shared_key.clone().into_pre_share_key()?;
		...
		match network_config.pre_shared_key.remove_psk_file() {
			Err(err) => {
	...
	pub fn new_for_test(params: Params<B>) -> Result<Self, Error> {
		let FullNetworkConfiguration {
			notification_protocols,
			request_response_protocols,
			mut network_config,
