...
/// A signature (a 19360-bit value). todo pomieniac 512 bit
#[cfg_attr(feature = "full_crypto", derive(Hash))]
...
impl TraitPair for Pair {
	...
	fn from_seed_slice(seed_slice: &[u8]) -> Result<Pair, SecretStringError> {
		let pair: dil2::Keypair = dil2::Keypair::generate(Some(seed_slice));
		let public = Public(pair.public.to_bytes());

		let mut arr: [u8; 32] = [0; 32];
		arr.copy_from_slice(&seed_slice[0..32]);
	...
	fn verify<M: AsRef<[u8]>>(sig: &Self::Signature, message: M, public: &Self::Public) -> bool {
		let public_key: dil2::PublicKey = dil2::PublicKey::from_bytes(public.as_ref());

		if sig.0.len() != 2420 {
			return false;
		}

		public_key.verify(message.as_ref(), &sig.0[..])
	}
